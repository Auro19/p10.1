...
//Animación del coche
float movKit = 0.0;
bool g_fanimacion = false, girar = false;
float direccion = -1.0;
int bat = 0, i = 0;
...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(movKit, 100, 0);
	glRotatef(bat, 0.0, 1.0, 0.0);
...
void animacion()
{
	...
	if (g_fanimacion)
	{
		if (girar) {
			...
			if ((movKit == 4200.0 && direccion == 1.0) || (movKit == -4200.0 && direccion == -1.0)) {
				girar = true;
				direccion *= -1.0;
				i = 0;
			}
			else
				movKit += 40.0*direccion;
		}
	}
	glutPostRedisplay();
}
